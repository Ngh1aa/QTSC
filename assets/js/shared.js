...
(() => {
  ...
  const searchItems = [
    ...
    { type:'Ecosystem', title:'Danh bạ doanh nghiệp', meta:'121 doanh nghiệp công nghệ số', url:'companies.html', keywords:'doanh nghiệp công ty member software data ai education telecom' },
    { type:'Ecosystem', title:'Chuỗi QTSC', meta:'Mô hình liên kết · thành viên · lợi ích', url:'qtsc-chain.html', keywords:'chuỗi qtsc chain thành viên lợi ích gia nhập' },
    ...
    { type:'Business', title:'Văn phòng tại QTSC', meta:'Office · Buildings · Meeting rooms', url:'office.html', keywords:'văn phòng office building phòng họp thuê' },
    { type:'Business', title:'Đầu tư tại QTSC', meta:'Lĩnh vực đầu tư · cơ hội mời gọi đầu tư', url:'investment.html', keywords:'đầu tư investment dự án lô chính sách ưu đãi' },
    { type:'Business', title:'Dịch vụ & hỗ trợ doanh nghiệp', meta:'Dịch vụ một cửa · thủ tục doanh nghiệp', url:'services.html', keywords:'dịch vụ một cửa hỗ trợ thủ tục visa giấy phép lao động' },
    { type:'Campus', title:'Khám phá QTSC', meta:'Map · 360 · Bus · Campus', url:'explore.html', keywords:'campus bản đồ map tour 360 bus r&d' },
    { type:'Campus', title:'Tiện ích nội khu', meta:'Ăn uống · ATM · Healthcare · Sports', url:'amenities.html', keywords:'tiện ích ăn uống cafe ngân hàng atm giáo dục thể thao y tế' },
    { type:'Insights', title:'Tin tức & hoạt động', meta:'News · Events · Member stories', url:'insights.html', keywords:'tin tức sự kiện hoạt động qtsc' },
    { type:'Careers', title:'Tuyển dụng QTSC', meta:'Việc làm · thực tập · doanh nghiệp', url:'careers.html', keywords:'tuyển dụng việc làm careers internship thực tập' },
    ...
    { type:'Contact', title:'Liên hệ QTSC', meta:'Địa chỉ · điện thoại · email', url:'contact.html', keywords:'liên hệ contact địa chỉ điện thoại email' }
  ];
  ...
  const megaLinkMap = {
    ...
    'Danh bạ thành viên':'companies.html','Tìm doanh nghiệp':'companies.html','QTSC Chain':'qtsc-chain.html','QTSC DigiTech Center':'technology-detail.html#innovation','DXCenter':'technology-detail.html#innovation','QTSC R&D Labs':'technology-detail.html#innovation','Giáo dục & cộng đồng':'about.html#ecosystem',
    'Văn phòng cho thuê':'office.html','Tòa nhà':'office.html#buildings','Phòng họp & hội nghị':'office.html','Đầu tư tại QTSC':'investment.html','Chính sách ưu đãi':'investment.html','Dịch vụ một cửa':'services.html#one-stop','Thủ tục doanh nghiệp':'services.html#one-stop',
    ...
    'Bản đồ QTSC':'explore.html','3D / 360 Tour':'explore.html#tour','Tòa nhà QTSC':'explore.html','Giao thông':'explore.html#mobility','Ăn uống & café':'amenities.html','Ngân hàng / ATM':'amenities.html','Giáo dục':'amenities.html','Thể thao & dịch vụ':'amenities.html',
    'Tin QTSC':'insights.html','Tin thành viên':'insights.html?cat=member','Sự kiện':'insights.html?cat=event','Tuyển dụng':'careers.html','Thông cáo báo chí':'insights.html','Hình ảnh & video':'insights.html','Tài liệu':'resources.html','Văn bản pháp luật':'resources.html?cat=legal'
  };
  ...
  const megaData = {
    ...
    explore:{label:'Khám phá',columns:[['Campus',['Bản đồ QTSC','3D / 360 Tour','Tòa nhà QTSC','Giao thông']],['Tiện ích',['Ăn uống & café','Ngân hàng / ATM','Giáo dục','Thể thao & dịch vụ']]],feature:['43 ha campus','Khám phá tòa nhà, tiện ích, giao thông và không gian công nghệ trong campus.']},
  ...
  const megaKeyByLabel = {'QTSC':'qtsc','Hệ sinh thái':'ecosystem','Doanh nghiệp':'business','Công nghệ':'technology','Khám phá':'explore','Tin & nguồn lực':'insights'};
  ...
  function renderMega(key) {
    ...
    content.innerHTML = data.columns.map(([title, links]) => `<div class="mega-col"><small>${title}</small>${links.map(label => `<a href="${megaLinkMap[label] || '#'}">${label}<span>↗</span></a>`).join('')}</div>`).join('') + `<aside class="mega-feature"><span>${data.label}</span><strong>${data.feature[0]}</strong><span>${data.feature[1]}</span></aside>`;
    enhanceInlineArrows(content);
  }
  ...
  function initMegaMenu() {
    ...
    $$('.nav-link', nav).forEach(link => {
      const key = megaKeyByLabel[link.textContent.trim()];
      if (!key) return;
      ...
      link.addEventListener('keydown', event => {
        if (window.matchMedia('(max-width: 900px)').matches) return;
        if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openMega(key, link); }
      });
    });
    header.addEventListener('mouseleave', closeMega);
    header.addEventListener('focusout', () => requestAnimationFrame(() => { if (!header.contains(document.activeElement)) closeMega(); }));
  }
  ...
  function enhanceInlineArrows(root = document) {
    $$('a, button', root).forEach(el => {
      ...
      icon.className = 'ui-arrow-icon'; icon.setAttribute('aria-hidden','true');
  ...
  function ensureSearch() {
    ...
    const overlay = document.createElement('div'); overlay.className = 'overlay'; overlay.id = 'searchOverlay'; overlay.setAttribute('aria-hidden','true');
    ...
    const input = $('#globalSearchInput', overlay), results = $('#globalResults', overlay);
    const render = () => {
      ...
      enhanceInlineArrows(results);
    };
    input.addEventListener('input', render); $$('[data-close-search]', overlay).forEach(el => el.addEventListener('click', closeSearch)); render(); return overlay;
  }
  ...
  function ensureContact() {
    ...
    const overlay = document.createElement('div'); overlay.className='overlay'; overlay.id='connectOverlay'; overlay.setAttribute('aria-hidden','true');
    ...
    document.body.append(overlay); enhanceInlineArrows(overlay);
    $$('[data-close-connect]', overlay).forEach(el => el.addEventListener('click', closeContact));
    $('#innerConnectForm', overlay).addEventListener('submit', event => { event.preventDefault(); $('#innerConnectForm', overlay).hidden = true; $('#innerFormSuccess', overlay).hidden = false; });
    return overlay;
  }
  ...
  function ensureMobileMenu() {
    ...
    const menu = document.createElement('div'); menu.className='mobile-menu'; menu.id='innerMobileMenu'; menu.setAttribute('aria-hidden','true');
    menu.innerHTML = `<div class="mobile-menu-head"><div class="brand-fallback" style="display:flex"><strong>QTSC</strong><small>Quality Tech Solution Complex</small></div><button class="close-btn" id="innerMobileClose" aria-label="Đóng menu">×</button></div><nav class="mobile-menu-content" aria-label="Điều hướng mobile"><div class="mobile-acc open"><button type="button">Khám phá QTSC <span>−</span></button><div class="mobile-acc-panel"><a href="about.html">Giới thiệu</a><a href="companies.html">Hệ sinh thái doanh nghiệp</a><a href="qtsc-chain.html">Chuỗi QTSC</a><a href="marketplace.html">Technology Marketplace</a><a href="office.html">Văn phòng</a><a href="investment.html">Đầu tư</a><a href="services.html">Dịch vụ & hỗ trợ</a><a href="explore.html">Campus</a><a href="amenities.html">Tiện ích nội khu</a><a href="insights.html">Tin & nguồn lực</a><a href="careers.html">Tuyển dụng</a><a href="resources.html">Download Center</a><a href="contact.html">Liên hệ</a></div></div></nav><div class="mobile-menu-foot"><button class="btn btn-brand" id="innerMobileConnect">Kết nối QTSC <span>↗</span></button><a class="btn" href="sitemap.html">Sitemap</a></div>`;
    document.body.append(menu); enhanceInlineArrows(menu);
    $('#innerMobileClose',menu).addEventListener('click',closeMobileMenu); $('#innerMobileConnect',menu).addEventListener('click',()=>{closeMobileMenu();openContact();}); return menu;
  }
  ...
  function contactNeedForPage() {
    const page = document.body.dataset.page || '';
    if (page === 'office') return 'Văn phòng / đặt lịch tham quan';
    if (page === 'technology-detail') return 'Tư vấn công nghệ';
    if (page === 'company-detail') return 'Kết nối doanh nghiệp';
    if (page === 'services') return 'Dịch vụ một cửa / hỗ trợ doanh nghiệp';
    if (page === 'investment') return 'Đầu tư tại QTSC';
    return 'Liên hệ QTSC';
  }

  function initShell() {
    initMegaMenu(); enhanceInlineArrows();
    $('#mobileMenuOpen')?.addEventListener('click', openMobileMenu);
    $('#searchOpen')?.addEventListener('click', openSearch);
    $$('[data-demo-contact],[data-open-connect]').forEach(button => button.addEventListener('click', event => { if (button.tagName === 'A') event.preventDefault(); openContact(contactNeedForPage()); }));
    const header = $('#siteHeader');
    const syncScroll = () => header?.classList.toggle('scrolled', window.scrollY > 16);
